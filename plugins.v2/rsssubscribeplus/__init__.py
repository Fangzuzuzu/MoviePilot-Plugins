...
class RssSubscribePlus(_PluginBase):
    # 插件名称
    plugin_name = "自定义订阅增强版"
    # 插件描述
    plugin_desc = "修改自 jxxghp，新增忽略媒体库已存在开关，库里已有也照样下载/订阅。"
    # 插件图标
    plugin_icon = "rss.png"
    # 插件版本
    ...
    # 插件配置项ID前缀
    plugin_config_prefix = "rsssubscribeplus_"
    # 加载顺序
    plugin_order = 19
    # 可使用的用户级别
    auth_level = 2
    ...
    def init_plugin(self, config: dict = None):
        ...
        if config:
            ...
            self._ignore_exists = config.get("ignore_exists")
                    ...
                    'content': [
                        {
                            ...
                            'events': {
                                'click': {
                                    'api': 'plugin/RssSubscribePlus/delete_history',
                                    'method': 'get',
                                    'params': {
                                        'key': title,
