...
MoviePilot 个人插件库，提供 PT 站点签到、自定义订阅等功能扩展。
...
### 自定义订阅增强版 (RssSubscribePlus)

修改自 [jxxghp/RssSubscribe](https://github.com/jxxghp/MoviePilot-Plugins/tree/main/plugins.v2/rsssubscribe) v2.1，新增「忽略媒体库已存在」开关。

**v2.1.1 更新内容：**
- ✅ 新增「忽略媒体库已存在」开关，开启后媒体库里已有的条目也会继续下载/订阅
- ✅ 类名重命名为 `RssSubscribePlus`，与官方插件并存、互不覆盖

**功能特性：**
- 🕒 定时刷新 RSS 报文，支持 5 位 cron 表达式
- 🧠 自动识别媒体信息（TMDB），按类型加入订阅或直接下载
- 🎯 支持包含 / 排除正则、种子大小范围过滤
- 🧰 支持订阅优先级规则、代理访问
- 🔁 新增开关：即使媒体库已有也照常处理（补种、重新入库场景）
- 🗂️ 历史记录页可查看与删除
...
2. 安装「自定义订阅增强版」插件
3. 填入一行或多行 RSS 地址
4. 设置包含 / 排除关键字（支持正则）与种子大小（如 `3` 或 `3-5` GB）
5. 选择动作：订阅 / 下载
6. 需要时打开「忽略媒体库已存在」开关
7. 启用插件并保存
